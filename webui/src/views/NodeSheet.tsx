...
import { api, useResource, type Channel, type Exchange, type Node, type NodeName } from "../api";
...
export default function NodeSheet({
  ...
  const nodesTick = useTick("nodes");
  const exchangesTick = useTick("exchanges");
  const nodes = useResource<Node[]>(() => api.nodes(), [nodesTick]);
  const channels = useResource<Channel[]>(() => api.channels(), []);
  const exchanges = useResource<Exchange[]>(() => api.exchanges(nodeNum, 20), [nodeNum, exchangesTick]);
  ...
  return (
    <Sheet label="Node" onClose={onClose} className="max-w-md">
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold text-mist-200">{n ? nodeLabel(n) : fullName(fallback)}</div>
      ...
      {!mine && (
        <section className="mt-4">
          ...
          <p className="mt-2 text-[11px] text-mist-400">
            The answer comes back over the mesh and can take minutes. A node without the channel picked here cannot read
            the question.
          </p>
        </section>
      )}
...
function outcome(e: Exchange): string {
  ...
  if (e.kind === "telemetry" && r.battery_level !== undefined) {
    return ` · ${batteryLabel(r.battery_level)}${r.voltage !== undefined ? `, ${r.voltage.toFixed(2)} V` : ""}`;
  }
...
function RouteLines({ exchange: e }: { exchange: Exchange }) {
  ...
  const leg = (start: string, names: NodeName[] | undefined, snr: (number | null)[] | undefined, end: string) => {
    const hops = names ?? [];
